// PropRisk Guard — script.js (FINAL PRODUCTION)
document.addEventListener('DOMContentLoaded', () => {
  // ====== CONFIG STRIPE ======
  // 1) Payment Link LIVE (Stripe):
  const STRIPE_LINK = "https://buy.stripe.com/9B6aEQc3IcLM2y18r683C00";
  // 2) Tokenul de deblocare din Success URL (setează în Stripe -> After payment -> Redirect to URL):
  //    https://propriskguard.com/?unlock=PRG-STRP-9F8DKZ-2025
  const UNLOCK_TOKEN = "PRG-STRP-9F8DKZ-2025";
  ...
  // ====== STRIPE: BUY & UNLOCK ======
  function gotoStripe() {
    if (!STRIPE_LINK) { alert('STRIPE_LINK nu este setat.'); return; }
  ...
  // Deblocare doar dacă tokenul din URL corespunde
  if (qs.has('unlock')) {
    const k = String(qs.get('unlock')||'').toUpperCase();
    if (k === UNLOCK_TOKEN) {
      try { localStorage.setItem('prg_unlock_from_url', k); } catch(e){}
    }
  }
  try {
    const k = String(localStorage.getItem('prg_unlock_from_url')||'').toUpperCase();
    if (k === UNLOCK_TOKEN) unlock();
  } catch(e){}

  // ====== LICENSE FALLBACK ======
  const licenseInput=$('license'), redeemBtn=$('redeem');
  redeemBtn?.addEventListener('click', ()=>{
    const k=(licenseInput?.value||'').trim().toUpperCase();
    if(k === UNLOCK_TOKEN){ unlock(); return; }
    if(!k){ gotoStripe(); return; }
    alert('Invalid license key.');
  });

  // Lock by default dacă nu e unlock deja
  ...
  const resetTimer=$('resetTimer'), coach=$('coach'), copyLotBtn=$('copyLot'), ckWarn=$('ckWarn');
  ...
  function currentEquity() {
    ...
    const ses = parseFloat(plSessionEl?.value || 0);
    return bal + pl + ses;
  }

  function setStartEquity(v) {
    startEquity = v;
    if (sodVal) sodVal.textContent = (typeof fmt === 'function') ? fmt(v) : v.toLocaleString();
  }
  ...
  function pushChartPoint() {
    ...
    const eqDs   = chart.data.datasets[0].data;
    const limDs  = chart.data.datasets[1].data;
  ...
  if (canvas) {
    ...
    const io = new IntersectionObserver((entries)=>{
      ...
      if (visible) startChartLoop(); else stopChartLoop();
    }, { threshold: .15 });
    io.observe(canvas);
  }
  ...
  // ====== Countdown + HUD ======
  function tick(){ const now=new Date(); const t=new Date(now); t.setHours(24,0,0,0);
    ...
    if(resetTimer) setText(resetTimer, txt); const hudTimer=$('hudTimer'); if(hudTimer) setText(hudTimer, txt);
  }

  // ====== Share / Print ======
  $('copyLink')?.addEventListener('click', async ()=>{ const q=new URLSearchParams({ bal:balanceEl?.value, pl:plTodayEl?.value, c:curCcy, mdl:mdlPctEl?.value, mol:molPctEl?.value, rp:riskPctEl?.value, sl:slPipsEl?.value, pip:pipValEl?.value }); const url=`${location.origin}${location.pathname}?${q}`; try{ await navigator.clipboard.writeText(url); alert('Link copied.'); }catch(e){ prompt('Copy link:', url); } });
  ...
  copyLotBtn?.addEventListener('click', async ()=>{
    ...
    const txt = $('lotSize')?.textContent || ''; try{ await navigator.clipboard.writeText(txt); copyLotBtn.textContent='Copied'; setTimeout(()=>copyLotBtn.textContent='Copy',1000); }catch(e){ alert('Copy failed'); }
  });
  ...
  function botReply(q){
    ...
    if(ql.includes('help')||ql==='?') return 'Try: <b>remaining</b>, <b>max daily</b>, <b>overall</b>, <b>lot</b>, <b>timer</b>, sau întreabă: “risk 1% with 30 pips”.';
    if(ql.includes('remaining')) return `You can still lose <b>${fmt(remaining)}</b> today before breaching the daily limit.`;
    if(ql.includes('max daily')) return `Max Daily Loss is <b>${fmt(maxDaily)}</b> for your current balance and rules.`;
  ...
  function send(){
    ...
    if(!t) return;
    addMsg(t,'me'); addMsg(botReply(t),'bot');
  ...
  (function fromQuery(){
    ...
    if(p.has('bal')&&balanceEl) balanceEl.value=p.get('bal');
    if(p.has('pl')&&plTodayEl)  plTodayEl.value=p.get('pl');
    if(p.has('c')&&ccyEl){ ccyEl.value=p.get('c'); setCurrency(p.get('c'), true); }
    if(p.has('mdl')&&mdlPctEl) mdlPctEl.value=p.get('mdl');
    if(p.has('mol')&&molPctEl) molPctEl.value=p.get('mol');
    if(p.has('rp')&&riskPctEl) riskPctEl.value=p.get('rp');
    if(p.has('sl')&&slPipsEl)  slPipsEl.value=p.get('sl');
    if(p.has('pip')&&pipValEl) pipValEl.value=p.get('pip');
  })();

  // ====== Init ======
  [balanceEl, plTodayEl, plSessionEl, riskPctEl, slPipsEl, pipValEl, mdlPctEl, molPctEl, alertPctEl]
    .forEach(el=> el?.addEventListener('input', calc));
  if(ccyEl) setCurrency(ccyEl.value, true);
  calc(); renderMatrix(); tick(); setInterval(tick, 1000);
});
